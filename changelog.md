...
- **战场单位列表**（我军 / 敌军）：副行不再显示 `(x,y)`；阵亡副行改为 **等级 + 兵种**。
- **武将信息**：阵亡检视说明去掉「坐标（…）」。
- **主战场格**：原生 `title` 仅保留地形名（若有），**不含格坐标**。
- **战局略图**：格子 `title` 为地形中文名，**不含坐标**。
- **战报**：移动结算句改为「某某 **完成移动**。」，不再带格坐标。

### 主战场鼠标静止时弱化提示

- 在 **`battle-map-viewport`** 上对指针活动做 **约 1.4s** 空闲判定：**静止后**隐藏左侧 **演义提要 / 胜利条件**；右侧「武将信息」内 **战场地形图例**；主棋盘上格的 **地形 title** 与属性浮窗 **「脚下 xxx」** 行。
- **换关**（`scenarioId` 变化）重新视为活跃并开始计时；在视口内 **移动 / 进入** 鼠标即恢复显示并重置计时。

### 战场单位列表：兵种与等级

- **我军 / 敌军**存活行：`Lv`、**将领种类**（`TROOP_KIND_LABEL`）、**兵力**。
- **阵亡**行：`Lv`、将领种类（状态标签「阵亡」等仍保留）。
